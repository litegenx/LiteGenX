...
// Copyright (c) 2015-2017 The litegenx developers
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat litegenx and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 LGX per transaction on average.
            */
            if (r <= 10) {
...
bool CCoinMixingPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeLiteGenXAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CCoinMixingPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100LGX+1 ( bit on if present )
    // bit 1 - 10LGX+1
    // bit 2 - 1LGX+1
    // bit 3 - .1LGX+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("litegenx-coinmixing");
