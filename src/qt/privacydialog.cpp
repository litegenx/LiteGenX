...
#include "zlitegenxcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zLGX ought to be enough for anybody." - Bill Gates, 2017
    ui->zLGXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzLGXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzLGX->setEnabled(false);
        ui->pushButtonMintzLGX->setToolTip(tr("zLGX is currently disabled due to maintenance."));

        ui->pushButtonSpendzLGX->setEnabled(false);
        ui->pushButtonSpendzLGX->setToolTip(tr("zLGX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zLGXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzLGX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLGX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zLGX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zLGX in ") +
...
void PrivacyDialog::on_pushButtonSpendzLGX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLGX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zLGX
        sendzLGX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zLGX
    sendzLGX();
}

void PrivacyDialog::on_pushButtonZlitegenxControl_clicked()
{
    ZlitegenxControlDialog* zlitegenxControl = new ZlitegenxControlDialog(this);
    zlitegenxControl->setModel(walletModel);
    zlitegenxControl->exec();
}

void PrivacyDialog::setZlitegenxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzlitegenxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzLGX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid litegenx Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zLGXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zLGX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " LGX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zLGX</b>";
    ...
    // use mints from zlitegenx selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZlitegenxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZlitegenxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zlitegenx selector in case it was used
    ZlitegenxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zlitegenx Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " litegenx, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zlitegenx Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zLGX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zLGX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zLGX "));
    ui->labelzLGXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzLGXSyncStatus->setVisible(fShow);
}
