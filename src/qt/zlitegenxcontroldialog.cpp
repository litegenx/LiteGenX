...
#include "ui_zlitegenxcontroldialog.h"
...
std::list<std::string> ZlitegenxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZlitegenxControlDialog::listMints;

ZlitegenxControlDialog::ZlitegenxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZlitegenxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZlitegenxControlDialog::~ZlitegenxControlDialog()
{
    delete ui;
}

void ZlitegenxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZlitegenxControlDialog::updateList()
{
...
void ZlitegenxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZlitegenxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZlitegenx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZlitegenxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZlitegenxControlDialog::GetSelectedMints()
{
...
void ZlitegenxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
