...
#if defined(NDEBUG)
#error "litegenx cannot be compiled without assertions."
#endif
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zlitegenx spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zlitegenx spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zlitegenx spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight >= 0 && nHeight <= Params().LAST_POW_BLOCK())
        nSubsidy = 10000 * COIN;
    else if (nHeight >= Params().LAST_POW_BLOCK() && nHeight <= 87840)
        nSubsidy = 11 * COIN;
    else if (nHeight >= 87840 && nHeight <= 348480)
        nSubsidy = 10 * COIN;
    else if (nHeight >= 348480 && nHeight <= 613440)
        nSubsidy = 9.5 * COIN;
    else if (nHeight >= 613440 && nHeight <= 874080)
        nSubsidy = 9 * COIN;
    else if (nHeight >= 874080 && nHeight <= 1139040)
        nSubsidy = 8.5 * COIN;
    else if (nHeight >= 1139040 && nHeight <= 1399680)
        nSubsidy = 8 * COIN;
    else if (nHeight >= 1399680 && nHeight <= 1664640)
        nSubsidy = 7.5 * COIN;
    else if (nHeight >= 1664640 && nHeight <= 1925280)
        nSubsidy = 7 * COIN;
    else if (nHeight >= 1925280 && nHeight <= 2190240)
        nSubsidy = 6.5 * COIN;
    else if (nHeight >= 2190240 && nHeight <= 2450880)
        nSubsidy = 6 * COIN;
    else if (nHeight >= 2450880 && nHeight <= 2715840)
        nSubsidy = 5.5 * COIN;
    else if (nHeight >= 2715840 && nHeight <= 2976480)
        nSubsidy = 5 * COIN;
    else if (nHeight >= 2976480 && nHeight <= 3241440)
        nSubsidy = 4.5 * COIN;
    else if (nHeight >= 3241440 && nHeight <= 3502080)
        nSubsidy = 4 * COIN;
    else if (nHeight >= 3502080 && nHeight <= 3767040)
        nSubsidy = 3.5 * COIN;
    else if (nHeight >= 3767040 && nHeight <= 4027680)
        nSubsidy = 3 * COIN;
    else if (nHeight >= 4027680 && nHeight <= 4292640)
        nSubsidy = 2.5 * COIN;
    else if (nHeight >= 4292640 && nHeight <= 4553280)
        nSubsidy = 2 * COIN;
    else if (nHeight >= 4553280 && nHeight <= 4818240)
        nSubsidy = 1.5 * COIN;
    else if (nHeight >= 4818240 && nHeight <= 5078880)
        nSubsidy = 1 * COIN;
    else
        nSubsidy = 1 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight >= Params().LAST_POW_BLOCK() && nHeight <= 87840)
        ret = blockValue * 0.50; // 50% MN
    else if (nHeight >= 87840 && nHeight <= 348480)
        ret = blockValue * 0.51; // 51% MN
    else if (nHeight >= 348480 && nHeight <= 613440)
        ret = blockValue * 0.52; // 52% MN
    else if (nHeight >= 613440 && nHeight <= 874080)
        ret = blockValue * 0.53; // 53% MN
    else if (nHeight >= 874080 && nHeight <= 1139040)
        ret = blockValue * 0.54; // 54% MN
    else if (nHeight >= 1139040 && nHeight <= 1399680)
        ret = blockValue * 0.55; // 55% MN
    else if (nHeight >= 1399680 && nHeight <= 1664640)
        ret = blockValue * 0.60; // 60% MN
    else if (nHeight >= 1664640 && nHeight <= 1925280)
        ret = blockValue * 0.61; // 61% MN
    else if (nHeight >= 1925280 && nHeight <= 2190240)
        ret = blockValue * 0.62; // 62% MN
    else if (nHeight >= 2190240 && nHeight <= 2450880)
        ret = blockValue * 0.63; // 63% MN
    else if (nHeight >= 2450880 && nHeight <= 2715840)
        ret = blockValue * 0.64; // 64% MN
    else if (nHeight >= 2715840 && nHeight <= 2976480)
        ret = blockValue * 0.65; // 65% MN
    else if (nHeight >= 2976480 && nHeight <= 3241440)
        ret = blockValue * 0.66; // 66% MN
    else if (nHeight >= 3241440 && nHeight <= 3502080)
        ret = blockValue * 0.67; // 67% MN
    else if (nHeight >= 3502080 && nHeight <= 3767040)
        ret = blockValue * 0.68; // 68% MN
    else if (nHeight >= 3767040 && nHeight <= 4027680)
        ret = blockValue * 0.69; // 69% MN
    else if (nHeight >= 4027680 && nHeight <= 4292640)
        ret = blockValue * 0.70; // 70% MN
    else if (nHeight >= 4292640 && nHeight <= 4553280)
        ret = blockValue * 0.71; // 71% MN
    else if (nHeight >= 4553280 && nHeight <= 4818240)
        ret = blockValue * 0.72; // 72% MN
    else if (nHeight >= 4818240 && nHeight <= 5078880)
        ret = blockValue * 0.73; // 73% MN
    else if(nHeight >= 5078880)  //After 10 Years
        ret = blockValue * 0.74; // 74% MN
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 1 * 60 * 60); // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from litegenx
...
{
    RenameThread("litegenx-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZLGXMinted()
{
...
void RecalculateZLGXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zLGX supply
...
bool RecalculateLGXSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if (!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zlitegenx with serial %s is already in the block %d\n",
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zlitegenxSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert LGX to zLGX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zlitegenx spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zlitegenx serial %s in block\n Block: %s",
...
bool AcceptBlockHeader(const CBlock& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            ...
            // if (pindex && Checkpoints::CheckBlock(pindex->nHeight - 1, block.hashPrevBlock)) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.GetHash() != Params().HashGenesisBlock()) {
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            //If this "invalid" block is an exact match from the checkpoints, then reconsider it
            //if (Checkpoints::CheckBlock(pindexPrev->nHeight, block.hashPrevBlock)) {
            //   LogPrintf("%s : Reconsidering block %s height %d\n", __func__, pindexPrev->GetBlockHash().GetHex(), pindexPrev->nHeight);
            //   CValidationState statePrev;
            //  ReconsiderBlock(statePrev, pindexPrev);
            //  if (statePrev.IsValid()) {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zlitegenx mints and %d zlitegenx spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // litegenx: We use certain sporks during IBD, so check to see if they are
